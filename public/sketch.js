...
socket.on('assignKey',function(key){
    const assignedKeyy = key;
    document.getElementById("assignedKey").innerHTML = assignedKeyy;
});
...
function moveRect(e) {
    ...
    switch (e.keyCode) {
        case 87: // W key
        if(assignedKey.innerHTML == 'W'){
            console.log(assignedKey.innerHTML);
            socket.emit('W');
        };
        break;
        case 65: // A key
        if(assignedKey.innerHTML=='A'){
            console.log(assignedKey.innerHTML);
            socket.emit('A');
        };
        break;
        case 83: // S key
        if(assignedKey.innerHTML=='S'){
            console.log(assignedKey.innerHTML);
            socket.emit('S');
        };
        break;
        case 68: // D key
        if(assignedKey.innerHTML=='D'){
            console.log(assignedKey.innerHTML);
            socket.emit('D');
        };
        break;
